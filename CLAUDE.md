...
- **`RedAnts-WebApp/src/RedAnts.Show`** (Razor class library) — the soundboard at `show[-dev].redants.ch` → `/show`, backoffice section "Show" (iframe to `/admin/show`), `/api/show` for the Companion module. Same layering as Ticketing: `Domain/Show/` (profiles, buttons, sounds, `ShowProfileRules`), `Features/Show/Ports/` (`IShowProfiles`, `IShowSettings`, `IShowRemote`, `IShowSpotifySearch`, `IShowSoundUploader`), `Features/Show/ShowWorkflow/` slices registered in `ShowFeatures.Handlers`, NPoco repositories in `Infrastructure/Show/` on the `show` schema (`ShowDatabase` uses Umbraco's scope provider unless `ConnectionStrings:showDbDSN` points elsewhere), `Show:Storage` blob options for sounds.
...
The verified status against the target architecture (pillar by pillar) and the recommended backlog live in ARCHITECTURE.md, section "Target architecture status and backlog"; update that section when a backlog item lands.
