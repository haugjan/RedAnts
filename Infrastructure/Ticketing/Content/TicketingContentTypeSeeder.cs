    ...
    ILogger<TicketingContentTypeSeeder> logger) : INotificationAsyncHandler<UmbracoApplicationStartedNotification>
{
    ...
    private void EnsureContentTypes()
    {
        ...
        evt.AddPropertyType(Prop(textBox, A.EventInternalLink, "Interner Link"), Group, GroupName);
        ...
        evt.AddPropertyType(Prop(decimalType, A.EventPriceChild, "Preis Kind"), "prices", "Preise");
        evt.AddPropertyType(Prop(decimalType, A.EventPriceYouth, "Preis Jugend"), "prices", "Preise");
        evt.AddPropertyType(Prop(decimalType, A.EventPriceAdult, "Preis Erwachsen"), "prices", "Preise");
    ...
    private void EnsureSampleContent()
    {
        ...
        openEvent.SetValue(A.EventPriceChild, 10m);
        openEvent.SetValue(A.EventPriceYouth, 15m);
        openEvent.SetValue(A.EventPriceAdult, 25m);
        Publish(openEvent);
        ...
        internEvent.SetValue(A.EventPriceAdult, 25m);
        Publish(internEvent);
        ...
        closedEvent.SetValue(A.EventPriceAdult, 25m);
        Publish(closedEvent);
