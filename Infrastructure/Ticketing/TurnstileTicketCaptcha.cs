...
/// <summary>Verifies Cloudflare Turnstile tokens on public purchase POSTs. Mirrors the reference adapter.</summary>
public sealed class TurnstileTicketCaptcha(IHttpClientFactory httpClientFactory, IConfiguration config) : ICaptcha
{
    ...
    public async Task<bool> VerifyAsync(string? token, string? remoteIp)
    {
        ...
        if (string.IsNullOrEmpty(secret)) return true; // not configured -> allow (dev)
