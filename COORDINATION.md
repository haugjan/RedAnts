...
| S2 | Flextickets-Slice: Menüpunkt + Bundle-Entität + Overlay (vom Nutzer freigegeben „S2 baut alles") | NEU: `Domain/Ticketing/Sales/FlexTicketBundle.cs`, `Infrastructure/Ticketing/Sales/FlexTicketRecords.cs`, `Infrastructure/Ticketing/Sales/FlexTicketBundleRepository.cs`, `Features/Ticketing/Ports/FlexTicketPorts.cs`, `Features/Ticketing/Admin/AdminFlexTicketsComponent.razor`. EDIT (additiv, mit S4 abgestimmt via Log): `Infrastructure/Ticketing/Sales/SalesRecords.cs` (+Spalte `SeasonSingleTickets.BundleId`), `Domain/Ticketing/Sales/SeasonSingleTicket.cs` (+`BundleId`/`CreateForBundle`), `Infrastructure/Ticketing/TicketingMigration.cs` (+Tabelle `FlexTicketBundles`), `Infrastructure/Ticketing/TicketingComposer.cs` (+DI-Zeile), `Features/Ticketing/Admin/TicketingAdminComponent.razor` (+Tab „Flextickets") | in Arbeit |
