...
|---|---|---|---|
| 30.07.2026 | S4 | **E-Mail-Texte im Backoffice pflegbar (Branch `feature/s4-mail-text-settings`).** Neuer Singleton-Content-Knoten **„E-Mail-Texte"** (Dokumenttyp `mailTexts`) unter der Ticketing-Root, idempotent im `TicketingContentTypeSeeder` geseedet (Methode `EnsureMailSettings`; Betreff = TextBox, Text = TextArea, je ein Paar für Helfereinladung/Mitgliederkarte/Saisonkarte; Aliases in `TicketingAliases`). Neuer Port **`ITicketingMailSettings`** (+ Enum `TicketingMailKind`) in `Features/Ticketing/Email`, Adapter `TicketingMailSettings` (liest den Knoten untyped). **`HelperInviteMailer`, `MemberCardMailer` und `SeasonPassMailer` bekommen je einen neuen Ctor-Parameter `ITicketingMailSettings settings`**; ihre `DefaultSubject`/`DefaultBody` lesen jetzt aus dem Knoten und fallen auf die bisherigen hartkodierten Texte zurück, wenn ein Feld leer ist. Kein DB-/NPoco-Schema. | Wer einen der drei Mailer konstruiert oder mockt: neuer Ctor-Parameter `ITicketingMailSettings` (in den `EmailComposer`-Registrierungen bereits gesetzt). Neuer Content-Knoten `mailTexts` wird beim Boot angelegt. Kein Verkaufs-Contract. |
