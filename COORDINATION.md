...
|---|---|---|---|
| 05.07.2026 | S1 | **Saison-Standards für neue Anlässe (in `main`, `522359e` + `d6b8f50`):** (1) Neuer Handler `EventPriceDefaults` (ContentSaved): ein Anlass OHNE eigenes `EventPrices` erhält beim Speichern einen Schnappschuss der Saison-Standards (ticket-angebotene Kategorien mit Standard-Ticketpreis/-Kontingent, Standard-Einlasskontingent als `AdmissionQuota`); nachträgliche Standard-Änderungen fassen bestehende Anlässe nicht an. (2) **Contract geändert:** `SeasonCategoryPrice` hat statt `Offered`/`Quota` jetzt getrennte Seiten: `PassOffered`/`PassPrice`/`PassQuota` (Saisonkarten-Verkauf, alte Spalten `Offered`/`SalePrice`/`Quota`, Semantik unverändert) und `TicketOffered`/`TicketPrice`/`TicketQuota` (Standard für neue Anlässe; **neue additive Spalten** `TicketOffered` backfilled von `Offered`, `TicketQuota`, Migrationsschritt `seasonprices-ticket-defaults`). `Create`/`FromPersistence`-Signaturen neu. Saisons-Preis-Overlay zeigt beide Seiten optisch getrennt. | **S4/S5**: Saisonkarten-Verkauf/Promo-Regel lesen weiterhin `Offered`/`Quota`-Spalten — unverändert. Wer `SeasonCategoryPrice` konstruiert: neue Signatur (Pass- und Ticket-Seite getrennt). |
