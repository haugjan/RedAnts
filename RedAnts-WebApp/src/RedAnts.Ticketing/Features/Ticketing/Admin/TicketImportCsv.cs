...
public sealed record TicketImportField(string Key, string Label);

public sealed record TicketImportTable(
...
public static class TicketImportCsv
{
    private static readonly string[] DateFormats = ["dd.MM.yyyy", "d.M.yyyy", "yyyy-MM-dd"];

    public static readonly IReadOnlyList<TicketImportField> Fields =
    [
        new("bundle", "Bundle"),
        new("category", "Kategorie"),
        new("admissions", "Anzahl"),
        new("company", "Firma"),
    ...
    public static TicketImportTable ReadTable(byte[] bytes, string fileName)
    {
        ...
        if (headerIndex < 0) return new TicketImportTable([], [], new Dictionary<string, int>());
        ...
        return new TicketImportTable(columns, rows, AutoMap(header));
    }
    ...
    public static TicketImportResult BuildRows(TicketImportTable table, IReadOnlyDictionary<string, int> map)
    {
        ...
        for (var r = 0; r < table.Rows.Count; r++)
        {
            ...
            var holder = CardHolder.Create(
                string.IsNullOrWhiteSpace(company) ? BuyerType.Private : BuyerType.Company,
                NormalizeSalutation(Get("salutation")), company, Get("firstname"), Get("lastname"),
                birthday, email, Get("street"), Get("addressline2"), Get("postalcode"),
                Get("city"), Get("country"), Get("phone"));

            var cardNo = Get("cardno");
            if (cardNo is null && holder.IsEmpty && Get("category") is null) continue;

            rows.Add(new TicketImportRow(cardNo, Get("bundle"), Get("category"), admissions, holder));
        }

        return new TicketImportResult(rows, warnings, errors);
    }
    ...
    private static string? HeaderKey(string? cell)
    {
        ...
        if (norm.Contains("email") || norm is "mail" or "emailadresse" or "primäreemail" or "primaereemail") return "email";
        return norm switch
        {
            "kartennr" or "kartennummer" or "karte" or "kartennrra" => "cardno",
            "bundle" or "referenz" or "reference" or "ref" or "gruppe" or "charge" or "teams" => "bundle",
            "kategorie" or "category" or "art" or "typ" => "category",
            ...
            "adresszusatz" or "zusatz" or "postfach" => "addressline2",
    ...
    {
        var csv = "Karten-Nr;Bundle;Kategorie;Anzahl;Firma;Anrede;Name;Vorname;Strasse;Adresszusatz;PLZ;Ort;Land;E-Mail;Telefon;Geburtsdatum\n" +
                  ";Sponsoren 2026;Erwachsen;;;Frau;Muster;Anna;Musterweg 1;;8400;Winterthur;Schweiz;anna.muster@example.com;079 000 00 00;14.05.1990\n" +
                  ";Sponsoren 2026;Jugend;;Beispiel AG;;;;Bahnhofstrasse 5;;8400;Winterthur;Schweiz;info@beispiel.ch;;\n";
