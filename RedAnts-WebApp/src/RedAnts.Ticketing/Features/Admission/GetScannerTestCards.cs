...
public static class GetScannerTestCards
{
    public sealed record Query;

    public sealed class Handler(ITicketTokens tokens, IQrCodeRenderer qr, IPublicBaseUrl publicUrl)
    {
        ...
        public Task<IReadOnlyList<TicketCardModel>> HandleAsync(Query query)
        {
            var baseUrl = publicUrl.Resolve();
            IReadOnlyList<TicketCardModel> cards = ExampleTypes.Select(type =>
            {
