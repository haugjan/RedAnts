...
{
    public async Task<Domain.Admission.Admission> LoadAsync(int eventId, TicketType ticketType, Guid ticketUuid)
    {
        ...
        if (rows.Count == 0) return Domain.Admission.Admission.None(eventId, ticketType, ticketUuid);
        ...
        return Domain.Admission.Admission.FromPersistence(eventId, ticketType, ticketUuid, visits);
    }

    public async Task SaveAsync(Domain.Admission.Admission admission)
    {
