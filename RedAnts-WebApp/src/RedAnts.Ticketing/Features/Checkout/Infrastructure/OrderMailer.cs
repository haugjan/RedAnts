    ...
    ILogger<OrderMailer> logger) : IOrderMailer
{
    ...
    private async Task<(string Subject, string Html, List<EmailAttachment> Images)> BuildAsync(OrderMailModel model)
    {
        ...
        var details = $"Bestellung {model.OrderNumber}\nTotal: CHF {RedAnts.Ticketing.Features.Shared.MoneyFormat.Chf(model.Total)}";
    ...
    private string TicketCard(OrderMailTicket ticket, string? dateText, string? venueText, int index, int total, List<EmailAttachment> images)
    {
        ...
        return counter +
            "<table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"ra-ticket\" style=\"margin:0 auto 14px;max-width:420px;background:#ffffff;border:1px solid #e6e8ec;border-radius:16px;overflow:hidden;page-break-inside:avoid;\">" +
                $"<tr><td style=\"background:{red};padding:14px 18px 15px;\">" +
                    "<table role=\"presentation\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\"><tr>" +
                        ...
                        "</td>" +
                        $"<td align=\"right\" width=\"64\" style=\"width:64px;padding-left:12px;vertical-align:top;\"><img src=\"{badgeCid}\" alt=\"Red Ants\" width=\"52\" height=\"52\" style=\"display:block;width:52px;height:52px;max-width:52px;border-radius:50%;\"></td>" +
                    "</tr></table>" +
                "</td></tr>" +
    ...
    private static string InfoRow(string key, string value, string valueColor) =>
        "<tr>" +
            ...
            $"<td align=\"right\" style=\"border-top:1px solid #eef0f2;padding:6px 0;font-family:Verdana,Geneva,Tahoma,sans-serif;color:{valueColor};font-size:14px;font-weight:700;\">{value}</td>" +
        "</tr>";
