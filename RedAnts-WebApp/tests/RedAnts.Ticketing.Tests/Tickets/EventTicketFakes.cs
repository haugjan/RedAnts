...
internal sealed class StubEventBundleTickets : IEventBundleTickets
{
    public Dictionary<int, List<EventBundleTicket>> Bundles { get; } = new();
    public IReadOnlyCollection<int>? LastBundleIds { get; private set; }

    public Task<IReadOnlyList<EventBundleTicket>> GetByBundleAsync(int bundleId) => GetByBundlesAsync([bundleId]);

    public Task<IReadOnlyList<EventBundleTicket>> GetByBundlesAsync(IReadOnlyCollection<int> bundleIds)
    {
        LastBundleIds = bundleIds;
        return Task.FromResult<IReadOnlyList<EventBundleTicket>>(bundleIds.SelectMany(id => Bundles.GetValueOrDefault(id) ?? []).ToList());
    }
}

internal sealed class StubTicketTokens : ITicketTokens
{
    public string Create(TicketType type, Guid uuid, int scopeId) => $"tok-{uuid:N}";

    public bool TryVerify(string token, out TicketTokenData data) => throw new NotSupportedException();

    public string CreateShort(Guid uuid) => $"s-{uuid.ToString("N")[..8]}";

    public bool TryVerifyShort(string token, out string code) => throw new NotSupportedException();
}
