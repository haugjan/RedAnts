...
public class AdmissionTests
{
    ...
    public void None_rejects_a_missing_event(int eventId) =>
        Assert.Throws<DomainException>(() => Admission.None(eventId, TicketType.EventTicket, Ticket));

    [Fact]
    public void None_rejects_free_entry_as_ticket_type() =>
        Assert.Throws<DomainException>(() => Admission.None(Event, TicketType.FreeEntry, Ticket));

    [Fact]
    public void First_check_in_creates_one_new_visit_with_origin_and_one_check_in_log()
    {
        var admission = Admission.None(Event, TicketType.EventTicket, Ticket);
    ...
    {
        var admission = Admission.None(Event, TicketType.SeasonPass, Ticket);
    ...
    {
        var admission = Admission.None(Event, TicketType.MemberCard, Ticket);
    ...
    public void Re_check_in_reuses_the_single_visit_for_non_member_types(TicketType type)
    {
        ...
        var admission = Admission.FromPersistence(Event, type, Ticket, [visit]);
    ...
    public void Check_ins_and_last_check_in_are_ordered_by_time_across_visits()
    {
        ...
        var admission = Admission.FromPersistence(Event, TicketType.MemberCard, Ticket, [newer, older]);
    ...
    public void Last_check_in_is_null_without_visits() =>
        Assert.Null(Admission.None(Event, TicketType.EventTicket, Ticket).LastCheckIn);
    ...
    public void From_persistence_orders_visits_by_id_and_logs_by_time()
    {
        ...
        var admission = Admission.FromPersistence(Event, TicketType.SeasonPass, Ticket, [late, early]);
