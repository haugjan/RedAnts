...
public class CartTests
{
    ...
    private static CartAddOn Parking(int seasonId = 7, bool mobile = false) =>
        new(1, "Parkplatz", Money.Of(20m), seasonId, "Saison 26/27", mobile);

    private static CartAddOn Locker(int seasonId = 7) =>
        new(2, "Garderobe", Money.Of(5m), seasonId, "Saison 26/27");
    ...
    public void AddEventTickets_merges_same_event_and_tier_and_refreshes_name_and_price()
    {
        ...
        cart.AddEventTickets(10, "Spiel A (neu)", 3, "Erwachsene", "Erw", Money.Of(30m), 1);
        ...
        Assert.Equal(30m, line.UnitPrice.Amount);
    ...
    {
        var cart = Cart.Empty();
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 1);
        cart.AddEventTickets(10, "Spiel A", 4, "Jugend", "Jug", Money.Of(15m), 1);
    ...
    {
        var cart = Cart.Empty();
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 40);
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 40);

        Assert.Equal(Cart.MaxQuantityPerLine, Assert.Single(cart.Items).Quantity);

        var fresh = Cart.Empty();
        fresh.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 999);
    ...
    {
        var cart = Cart.Empty();
        Assert.Throws<DomainException>(() => cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), quantity));
    ...
    {
        var cart = Cart.Empty();
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, [Parking()]);
        cart.AddSeasonPasses(7, "Saison 26/27", 3, "Erwachsene", "Erw.", Money.Of(320m), 2, [Parking(mobile: true)]);

        var line = Assert.Single(cart.Items);
        Assert.Equal(CartLineKind.SeasonPass, line.Kind);
        Assert.Equal(3, line.Quantity);
        Assert.Equal("Saison 26/27", line.EventName);
        Assert.Equal(320m, line.UnitPrice.Amount);
    ...
    public void AddSeasonPasses_with_different_add_on_sets_creates_separate_lines()
    {
        var cart = Cart.Empty();
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, [Parking()]);
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, []);
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, [Locker(), Parking()]);
    ...
    public void AddSeasonPasses_rejects_non_positive_quantity() =>
        Assert.Throws<DomainException>(() => Cart.Empty().AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 0, []));
    ...
    public void AddConversion_adds_one_per_call_up_to_the_cap()
    {
        ...
        Assert.Equal(1, cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), origin));
        Assert.Equal(1, cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), origin));
        Assert.Equal(0, cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), origin));
    ...
    {
        var cart = Cart.Empty();
        cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), Origin(CardA, cap: 500));
    ...
    public void AddConversion_never_merges_with_regular_lines_of_the_same_tier()
    {
        var cart = Cart.Empty();
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 1);
        cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), Origin(CardA));
        cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), Origin(CardB));
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 1);
    ...
    {
        var cart = Cart.Empty();
        cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), Origin(CardA, cap: 3));
        var key = cart.Items[0].Key;
    ...
    public void Removing_the_last_pass_of_a_season_prunes_its_order_add_ons()
    {
        var cart = Cart.Empty();
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, []);
        cart.AddSeasonPasses(8, "Saison alt", 3, "Erwachsen", "Erw", Money.Of(300m), 1, []);
    ...
    public void Totals_include_line_add_ons_and_order_add_ons()
    {
        var cart = Cart.Empty();
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 2);
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 2, [Parking()]);
        cart.AddOrderAddOns([Locker()]);

        Assert.Equal(5, cart.TotalQuantity);
        Assert.Equal(50m + 2 * 320m + 5m, cart.TotalAmount.Amount);
        Assert.Equal(640m, cart.Items[1].LineTotal.Amount);
        Assert.Equal(20m, cart.Items[1].AddOnTotal.Amount);
    }
    ...
    public void QualifiesForExpress_needs_a_small_pass_free_cart()
    {
        ...
        small.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(24.99m), 2);
        Assert.True(small.QualifiesForExpress);

        var atLimit = Cart.Empty();
        atLimit.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 2);
        Assert.False(atLimit.QualifiesForExpress);

        var withPass = Cart.Empty();
        withPass.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(10m), 1, []);
    ...
    {
        var none = Cart.Empty();
        none.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, [Locker()]);
        Assert.False(none.RequiresMobileNumber);

        var viaLine = Cart.Empty();
        viaLine.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, [Parking(mobile: true)]);
        Assert.True(viaLine.RequiresMobileNumber);

        var viaOrder = Cart.Empty();
        viaOrder.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, []);
    ...
    public void EventIds_and_SeasonIds_are_distinct_per_kind()
    {
        ...
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 1);
        cart.AddEventTickets(10, "Spiel A", 4, "Jugend", "Jug", Money.Of(15m), 1);
        cart.AddConversion(11, "Spiel B", 7, 3, "Erwachsen", Money.Of(0m), Origin(CardA));
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, []);
        cart.AddSeasonPasses(7, "Saison", 4, "Jugend", "Jug", Money.Of(200m), 1, []);
    ...
    public void EventDemand_lists_tier_quantities_and_conversion_flags_per_event()
    {
        ...
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 2);
        cart.AddConversion(10, "Spiel A", 7, 3, "Erwachsen", Money.Of(0m), Origin(CardA));
        cart.AddEventTickets(11, "Spiel B", 4, "Jugend", "Jug", Money.Of(15m), 1);
    ...
    public void PassDemand_lists_pass_lines_of_the_season()
    {
        ...
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 2, [Parking()]);
        cart.AddSeasonPasses(7, "Saison", 3, "Erwachsen", "Erw", Money.Of(300m), 1, []);
        cart.AddEventTickets(10, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 5);
    ...
    {
        var line = CartLine.FromPersistence(CartLineKind.EventTicket, 10, 0, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(25m), 0);
        var conversion = CartLine.FromPersistence(CartLineKind.EventTicket, 10, 7, "Spiel A", 3, "Erwachsen", "Erw", Money.Of(0m), 2, null, Origin(CardA));
    ...
    {
        var line = CartLine.FromPersistence(CartLineKind.SeasonPass, 0, 7, null!, 3, null!, null!, Money.Of(300m), 1);
