...
public class SetSeasonPricingTests
{
    ...
    private readonly StubTierSales _sales = new();
    ...
    public async Task New_tiers_get_ids_and_their_categories_point_at_the_saved_tiers()
    {
        ...
        var handler = new SetSeasonPricing.Handler(tiers, prices, _sales);
    ...
    public async Task Blank_names_are_ignored_and_positions_define_the_sort_order()
    {
        ...
        await new SetSeasonPricing.Handler(tiers, prices, _sales).HandleAsync(new SetSeasonPricing.Command(SeasonId,
        [
            Tier(0, "  "),
            Tier(0, "Erwachsen"),
            Tier(0, "Kind")
    ...
    public async Task An_existing_price_row_keeps_its_quotas_and_reservation()
    {
        ...
        await new SetSeasonPricing.Handler(tiers, prices, _sales).HandleAsync(new SetSeasonPricing.Command(SeasonId, [Tier(7, "Erwachsen")]));
    ...
    public async Task Removing_a_tier_with_sales_is_rejected_before_anything_is_saved()
    {
        ...
        var ex = await Assert.ThrowsAsync<DomainException>(() =>
            new SetSeasonPricing.Handler(tiers, prices, _sales).HandleAsync(new SetSeasonPricing.Command(SeasonId, [Tier(7, "Erwachsen")])));
    ...
    public async Task Tiers_of_other_seasons_are_left_alone()
    {
        ...
        await new SetSeasonPricing.Handler(tiers, prices, _sales).HandleAsync(new SetSeasonPricing.Command(SeasonId, [Tier(0, "Erwachsen")]));
