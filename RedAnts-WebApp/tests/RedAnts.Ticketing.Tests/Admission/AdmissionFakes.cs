...
internal sealed class InMemoryAdmissions : IAdmissionRepository
{
    ...
    private readonly Dictionary<(int EventId, Guid Uuid), Domain.Admission.Admission> _stored = [];

    public int Saves { get; private set; }

    public Task<Domain.Admission.Admission> LoadAsync(int eventId, TicketType ticketType, Guid ticketUuid) =>
        Task.FromResult(_stored.TryGetValue((eventId, ticketUuid), out var admission)
            ? admission
            : Domain.Admission.Admission.None(eventId, ticketType, ticketUuid));

    public Task SaveAsync(Domain.Admission.Admission admission)
    {
        Saves++;
    ...
    public Domain.Admission.Admission? Stored(int eventId, Guid uuid) => _stored.GetValueOrDefault((eventId, uuid));
}
