...
**Done 2026-09-09.** Commits `84b8bb9` (port and adapter), `7cc31e5` (`PlaceOrder` and `OrderFulfillment`), `134fc7a` (`CreateAdminOrder` and `RefundOrder`), `cfd591e` (the four imports) and the docs commit of branch `feature/s6-unit-of-work`. `IUnitOfWork` sits in `Features/Checkout` with a generic and a void overload plus a `CancellationToken`; `ScopedUnitOfWork` (`Features/Checkout/Infrastructure`) opens one `ICoreScopeProvider.CreateCoreScope()`, and because Umbraco dooms a parent scope silently when a child is disposed without `Complete()`, it throws a `DomainException` when `Complete()` returns false instead of reporting a write that never happened. Deviations from the steps above: the card mailings were not wrapped because no sent flag exists in the code, they only enqueue one outbox row and are therefore a single write; the best-effort `OrderItems` write of `OrderFulfillment` stays outside the scope with its swallowed exception, because a failure there must not roll a paid order back (`OrderItemsBackfill` covers it) and a swallowed exception inside the scope would doom it silently; the reservation release of `OrderFulfillment` moved from before the ticket issuing to after the commit, so a rolled-back fulfilment keeps its reservation; `RefundOrder` splits its Payrexx branch into the pending refund row, the gateway call and a scope around confirm plus ticket deactivation, so no transaction is open while Payrexx answers; the imports already opened one auto-completing scope per import, which commits even when a row throws, so the wrap is what makes a failing row roll the whole import back. Verified with 604 Ticketing unit tests (18 new), the architecture, Kernel, Show and Host suites, `CheckoutShould` and `AdminOrdersShould` against the agent database, a manual order placed through the local site (`AGT-2026-000122`, listed as paid under Bestellungen) and a manual cash refund of it through the admin (`2026-R000016`, order now Erstattet). Not verified live: an admin import upload.
...
**Done 2026-09-09.** Commits `f0e5dd8` (`CanCheckout`), `0e80ab2` (`CanRefund`), `0c02955` (`CanConvert`), `5f23dfc` (browser test for the refund dialog) and the docs commit of branch `feature/s6-checks`. The rules live in the aggregates (`Cart.CheckoutBlocker(CheckoutContext)`, `Cart.ConversionBlocker(eventId, cardUuid, cap, eventRemaining)`, `Order.RefundBlocker(remaining, amount, viaPayrexx, payrexxEnabled)`) with the reason records `CheckoutDenied`, `ConversionDenied` and `RefundDenied` carrying the German messages; `PlaceOrder`, `RefundOrder` and `AddConversionToCart` deny through the same methods. Deviations from the steps above: `CanConvert` sits in `Features/Checkout` and not in `MemberCards`, because it reads the cart caps through `ICartRepository` and the reverse dependency would close a cycle between the two capabilities; the facts a checkout rule needs are gathered once by the step `CheckoutEligibility` (registered in `TicketingFeatures.Steps`), which both `PlaceOrder` and `CanCheckout` call, so `PlaceOrder` no longer injects the occupancy reader and the conversion rules; the express eligibility became a fourth reason (`ExpressUnavailable`) that `PlaceOrder` denies without sending the buyer back to the cart; `Order.RequireRefundable` was removed in favour of the blocker; `AddConversionToCart.Result` lost `NeedsTier` and `TierChoices` because the tier choice is now the denial reason `CanConvert.TierChoiceRequired` that `CartController` answers with. UI: `/checkout` and `/checkout/express` show the reason and disable the pay button, `RefundOverlay` asks the check on open and before booking instead of repeating the amount rules, and `/cart/convert` asks the check before the command. Test state: Kernel 34, Show 19, Host 5, Ticketing 640 passed with 12 skipped without the agent database (31 new tests), Architecture 27, Browser 26 (`CheckoutShould` and `AdminOrdersShould` run green against the agent database on port 5611).
...
**Done 2026-09-10.** Commits `eea68ef` (kernel: `Money` with `Chf`/`Of`/`Stored`, comparisons and `Sum`, `EmailAddress` with a strict ASCII pattern, `Optional` and `IsValid`, 64 kernel tests), `c75628d` (`Order`, `OrderItem`, `OrderRefund`, `AccountingJournalEntry`, `RefundSummary`, `RefundRequest`, `CanRefund.Check` and `AdminOrderLine` on `Money`), `1387024` (`CartLine`, `CartAddOn`, `Cart.TotalAmount` and `ExpressLimit` on `Money`), `da5c4fa` (`CategoryPrice`, `SeasonCategoryPrice` and `SeasonAddOn` on `Money` with five-rappen snapping) and `94a61a6` (`EmailAddress` on `BillingAddress`, `Helper`, `NewsletterSignup`, `MemberCard`, `SeasonPass` and `CardHolder`; the format checks in `CheckoutController`, `TicketImportCsv`, `MemberContactRules` and the member card dialogs removed; new `Birthdate` guard). Deviations from the steps above: `Money.Chf` snaps only prices that a person types, while derived amounts keep rappen precision through `Money.Of` and stored decimals stay untouched through `Money.Stored`, because the five-rappen grid must not silently re-round a VAT split or a persisted value; `ValidationException` now derives from `DomainException` so every existing `catch (DomainException)` in the dialogs and controllers renders a field message without a second code path; the CSV import warns and drops an invalid address instead of failing the row; the prices on `EventTicket`, `SeasonPass` and `SeasonSingleTicket` stay `decimal` (they were not in the phase list and are read by exports, mails and the PDF renderer). The agent database was checked before the rounding change: every stored price, order total, item price and refund amount is already a multiple of 0.05 with at most two decimals, so nothing changes meaning. Test state: Kernel 64, Show 19, Host 5, Ticketing 645 with 12 database tests skipped, Architecture 27; the browser tests were left to the integrator.
